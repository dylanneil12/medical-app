...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.pacientes.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="nombres">{{ trans('cruds.paciente.fields.nombres') }}</label>
                <input class="form-control {{ $errors->has('nombres') ? 'is-invalid' : '' }}" type="text" name="nombres" id="nombres" value="{{ old('nombres', '') }}">
            ...
            <div class="form-group">
                ...
                @foreach(App\Paciente::SEXO_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('sexo') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="sexo_{{ $key }}" name="sexo" value="{{ $key }}" {{ old('sexo', '') === (string) $key ? 'checked' : '' }}>
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('padecimiento') ? 'is-invalid' : '' }}" name="padecimiento" id="padecimiento">{{ old('padecimiento') }}</textarea>
